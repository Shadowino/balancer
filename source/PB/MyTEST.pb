...
Procedure SerialConnect(x.i) ;procedure thread
  ...
  While 1
    ...
    If SendArd = 1 ; send arduino message if available
      ...
      Print("\tsend..." + Hex(ARmsg(4)) +" | "+ Hex(ARmsg(5)))
  ...
  If eve = #PB_Event_Gadget
    Select EventGadget()
      Case 6 ; "Send Parameters" button, create and send arduno message
        ...
        ARmsg(3) = $00FF & (Val(GetGadgetText(3)) * 182)
        ARmsg(4) = ($FF00 & (Val(GetGadgetText(3)) * 182)) >> 8
        ARmsg(5) = $00FF & (Val(GetGadgetText(4)) * 182)
        ARmsg(6) = ($FF00 & (Val(GetGadgetText(4)) * 182)) >> 8
